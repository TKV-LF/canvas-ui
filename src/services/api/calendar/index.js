...
const CalendarApi = () => {
	const getAllEvents = async () => {
		try {
			const response = await apiClient.get("/api/v1/calendar_events");
	...
	const setCourseTimetable = async (payload) => {
		try {
			const response = await apiClient.put(`/api/v1/courses/${payload.courseId}/calendar_events/timetable`, payload);
			return response.data;
		} catch (error) {
			// Rethrow the error to allow error handling further up the call stack
			throw error;
	...
	const createEventsDirectly = async (payload) => {
		try {
			const response = await apiClient.post(`/api/v1/courses/${payload.courseId}/calendar_events/timetable_events`, payload);
			return response.data;
		} catch (error) {
			// Rethrow the error to allow error handling further up the call stack
			throw error;
